...
class ScrollableLeaderboard(tk.Frame):
    def __init__(self, parent,width=550,height=550):
        ...
        self.canvas = tk.Canvas(self,width=width,height=height,bg="#222")
    ...
    def update_leaderboard(self, data):
        ...
        for index, (name, score) in enumerate(data):
            tk.Label(self.scrollable_frame,font=("Arial", 17),bg="#222",fg="snow",text=f"{index + 1}.   {name[0]}  |  {name[1]}  |  {score}                                                        ").pack(anchor="w", pady=2,)
...
class DropdownMenu(ttk.Combobox):
    def __init__(self, parent, values, label_text="CLUSTER  :", width=40):
        ...
        self.label = tk.Label(parent,font='Arial 17 bold', text=label_text,fg="snow",bg="#111")
